...
-   Upgraded @scaldwell77/aws-signed-fetch to ^5.1.0, so the signed request preserves the caller's `signal` (as well as `redirect` and `credentials`). Aborts and timeouts now cancel the in-flight request.
